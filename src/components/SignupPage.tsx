...
const API_URL = import.meta.env.PROD ? 'https://youtube-watch-party-backend-fsyd.onrender.com' : 'http://localhost:3001';
      ...
      <div className="w-full max-w-[600px] px-6 relative z-10 flex flex-col items-center mt-[-40px]">
        ...
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-[12px] bg-gradient-to-tr from-[#8a42ff] to-[#6d81f5] flex items-center justify-center shadow-[0_0_20px_rgba(138,66,255,0.4)] border border-white/20">
        ...
        >
          <div className="mb-10 text-center">
            <h1 className="text-4xl font-bold tracking-wide text-white mb-3">Create Account</h1>
            <p className="text-white/60 text-base font-medium">Join the ultimate synchronized viewing experience</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-7">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="group w-full py-4 px-6 bg-white hover:bg-zinc-200 text-black font-bold text-lg rounded-full shadow-[0_10px_30px_rgba(255,255,255,0.15)] transition-all duration-200 flex items-center justify-center disabled:opacity-70 disabled:cursor-not-allowed mt-4"
            >
              {isLoading ? (
                <div className="w-5 h-5 border-2 border-black/30 border-t-black rounded-full animate-spin"></div>
              ) : (
                <div className="flex items-center gap-2">
                  Sign Up <ArrowRight className="w-[18px] h-[18px] transition-transform group-hover:translate-x-1" />
                </div>
              )}
            </button>
          </form>
